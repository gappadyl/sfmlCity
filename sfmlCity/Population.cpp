...
{
	std::vector<sf::Vector2i> player_path; 
	...
	for (Entity* entity : population)
	{
		...
		{
			flood_fill(3, 3, fillMap, 0); 
			
			

				
			
				for (int y = 0; y < fillMap.size(); y++)
				{
					for (int x = 0; x < fillMap.size();x++)
					{
						std::cout << fillMap[x][y] << " "; 
					}
					std::cout << "\n"; 
				}

				LinkedPathList* newList = new LinkedPathList(); 
				 pathFinder(9, 10, fillMap, newList); 

				newList->printPath(); 
		}
		/*for (sf::Vector2i cordinate : player_path)
		{
			
			std::cout << "cordinate x: " << cordinate.x << " cordinate y: " << cordinate.y << std::endl; 
		}*/
	}
